...
const H = require('./rule-audit-source/tests/helpers.js');
const dbs = require('./rule-audit-source/cards.json');
const results = [];
...
scenario('P27','Actual SD01-020 without an extra GEM payment',()=>{const {E,st}=world();const m=H.mkInst(H.rowFor('SD01-020'),0);H.giveHand(st,0,[m]);return E.playMagic(st,0,m.uid,{});});
